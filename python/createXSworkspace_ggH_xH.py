...
def createXSworkspace(obsName, channel, nBins, obsBin, observableBins, usecfactor, addfakeH, modelName, physicalModel, year, obs_ifJES, obs_ifJES2):
    ...
    for genbin in range(nBins):
        ...
        if( not (obs_ifJES) or (not doJES)) :
            ...
            xH_norm[genbin] = RooFormulaVar("trueH"+channel+"Bin"+str(genbin)+"_norm","@0*@1*(1-@2)", RooArgList(fideff_var[genbin], lumi, JES_sig_rfv) );
...
##################################
    for genbin in range(nBins):
        if (physicalModel=="v3"):
            ...
            fracXH4eBin[str(genbin)] = RooRealVar('fracXH4eBin'+str(genbin), 'fracXH4eBin'+str(genbin), fidxs['4e']/fidxs['4l'])
            fracXH4eBin[str(genbin)].setConstant(True)
            fracXH4muBin[str(genbin)] = RooRealVar('fracXH4muBin'+str(genbin), 'fracXH4muBin'+str(genbin), fidxs['4mu']/fidxs['4l'])
            ...
            SigmaBin_xh[str(genbin)] = RooRealVar('SigmaBin_xh'+str(genbin), 'SigmaBin_xh'+str(genbin), fidxs['4l'], 0.0, 10.0)
            SigmaXHBin['4e'+str(genbin)] = RooFormulaVar("SigmaXH4eBin"+str(genbin),"(@0*@1*@2)", RooArgList(SigmaBin_xh[str(genbin)], fracXH4eBin[str(genbin)], K1XHBin[str(genbin)]))
            SigmaXHBin['4mu'+str(genbin)] = RooFormulaVar("SigmaXH4muBin"+str(genbin),"(@0*(1.0-@1*@2)*@3*@4/(1.0-@1))", RooArgList(SigmaBin_xh[str(genbin)], fracXH4eBin[str(genbin)], K1XHBin[str(genbin)], K2XHBin[str(genbin)], fracXH4muBin[str(genbin)]))
            SigmaXHBin['2e2mu'+str(genbin)] = RooFormulaVar("SigmaXH2e2muBin"+str(genbin),"(@0*(1.0-@1*@2)*(1.0-@3*@4/(1.0-@1)))", RooArgList(SigmaXHBin[str(genbin)], fracXH4eBin[str(genbin)], K1XHBin[str(genbin)], K2XHBin[str(genbin)], fracXH4muBin[str(genbin)]))
            #if (obsName == "nJets" or ("jet" in obsName)):
            #if (obs_ifJES):
            #    trueH_norm_final[genbin] = RooFormulaVar("trueH"+channel+"Bin"+str(genbin)+recobin+"_final","@0*@1*@2*(1-@3)" ,RooArgList(SigmaHBin[channel+str(genbin)],fideff_var[genbin],lumi,JES_sig_rfv))
            #else:
            #    trueH_norm_final[genbin] = RooFormulaVar("trueH"+channel+"Bin"+str(genbin)+recobin+"_final","@0*@1*@2" ,RooArgList(SigmaHBin[channel+str(genbin)],fideff_var[genbin],lumi))
            xH_norm_final[genbin] = RooFormulaVar("xH"+channel+"Bin"+str(genbin)+recobin+"_final","@0*@1*@2" ,RooArgList(SigmaXHBin[channel+str(genbin)],fideff_var[genbin],lumi))
        elif (physicalModel=="v1"):
            ...
            rXHBin[str(genbin)] = RooRealVar("rXHBin"+str(genbin),"rXHBin"+str(genbin), 1.0, 0.0, 10.0)
            rXHBin[str(genbin)].setConstant(True)
            #if (obsName == "nJets" or ("jet" in obsName)):
            #if (obs_ifJES):
            #    trueH_norm_final[genbin] = RooFormulaVar("trueH"+channel+"Bin"+str(genbin)+recobin+"_final","@0*@1*@2*@3*(1-@4)" ,RooArgList(rBin[str(genbin)],fracBin[channel+str(genbin)],fideff_var[genbin],lumi,JES_sig_rfv))
            #else:
            #    trueH_norm_final[genbin] = RooFormulaVar("trueH"+channel+"Bin"+str(genbin)+recobin+"_final","@0*@1*@2*@3" ,RooArgList(rBin[str(genbin)],fracBin[channel+str(genbin)],fideff_var[genbin],lumi))
            xH_norm_final[genbin] = RooFormulaVar("xH"+channel+"Bin"+str(genbin)+recobin+"_final","@0*@1*@2*@3" ,RooArgList(rXHBin[str(genbin)],fracXHBin[channel+str(genbin)],fideff_var[genbin],lumi))
        else:
            rXHBin_channel[str(genbin)] = RooRealVar("rXH"+channel+"Bin"+str(genbin),"rXH"+channel+"Bin"+str(genbin), 1.0, 0.0, 10.0)
            rXHBin_channel[str(genbin)].setConstant(True)
            #if (obsName == "nJets" or ("jet" in obsName)):
            #if (obs_ifJES):
            #    trueH_norm_final[genbin] = RooFormulaVar("trueH"+channel+"Bin"+str(genbin)+recobin+"_final","@0*@1*@2*(1-@3)", RooArgList(rBin_channel[str(genbin)], fideff_var[genbin],lumi,JES_sig_rfv))
            #else:
            #    trueH_norm_final[genbin] = RooFormulaVar("trueH"+channel+"Bin"+str(genbin)+recobin+"_final","@0*@1*@2", RooArgList(rBin_channel[str(genbin)], fideff_var[genbin],lumi))
            xH_norm_final[genbin] = RooFormulaVar("xH"+channel+"Bin"+str(genbin)+recobin+"_final","@0*@1*@2", RooArgList(rXHBin_channel[str(genbin)], fideff_var[genbin],lumi))
