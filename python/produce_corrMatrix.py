...
#                XH = []
                for obsBin in range(nBins):
                    ...
                    if(obsName!='mass4l'): cmd += 'SigmaBin'+str(obsBin)+'=1,' 
                    if(obsName=='mass4l'): cmd += 'SigmaBin'+str(obsBin)+'=2.86,'
                cmd = cmd[:-1]
            print cmd, '\n'
            output = processCmd(cmd)

        processCmd('mv robustHesse_'+obsName+'_'+physicalModel+opt.BKG+'.root combineOutputs/allYear/') # wanted combine output
...
def PlotCorrelation():
    for physicalModel in PhysicalModels:
        ...
        pars = od()
        inFile    = ROOT.TFile('combineOutputs/allYear/robustHesse_'+obsName+'_'+physicalModel+opt.BKG+'.root','READ')
        ...
        plt.savefig('combineOutputs/allYear/corr_'+obsName+'_'+physicalModel+opt.BKG+'.pdf', bbox_inches='tight')
