...
def createXSworkspace(obsName, channel, nBins, obsBin, observableBins, usecfactor, addfakeH, modelName, physicalModel,year,bkg):
    #import os,sys


#    sys.path.append('./datacardInputs_'+year+'/')
    sys.path.append('./datacardInputs_'+year+bkg+'/')


    print "workspace: working path is:      ",os.getcwd()
    obsBin_low = observableBins[obsBin]
    obsBin_high = observableBins[obsBin+1]
    print "obsBin_high:      ",obsBin_high
...
#        _temp = __import__('inputs_sig_'+obsName, globals(), locals(), ['cfactor','inc_wrongfrac','binfrac_wrongfrac','inc_outfrac','binfrac_outfrac','lambdajesup','lambdajesdn'], -1)
    ...
    # Load the legacy f
    '''
    workspace_in = TFile("125.0/hzz4l_"+channel+"S_8TeV.input.root","READ")  # for the moment TJ
    ...
    '''
    #if (bkg!=""):mass4l_low=105.0;mass4l_high=160.0;
    if (obsName=="mass4l"):mass4l_low=105.0;mass4l_high=float(obsBin_high);
    else: mass4l_low=105.0;mass4l_high=140.0;
    m = RooRealVar("CMS_zz4l_mass", "CMS_zz4l_mass", mass4l_low, mass4l_high)  #TJ
    MH = RooRealVar("MH","MH", 125.38, mass4l_low,mass4l_high)  # #TJ
    m.setMin(mass4l_low)
    m.setMax(mass4l_high)
    #m.setBins(45)
    #m.Print("v")

    mass4e = RooRealVar("mass4e", "mass4e", mass4l_low, mass4l_high)
    mass4mu = RooRealVar("mass4mu", "mass4mu", mass4l_low, mass4l_high)
    mass2e2mu = RooRealVar("mass2e2mu", "mass2e2mu",mass4l_low, mass4l_high)
    '''
    m.setMin(105.0)
    m.setMax(140.0)
    #m.setBins(45)
    ...
    # Wrong signal combination events
    if (channel=='4mu'):
        p1_1 = RooRealVar("CMS_fakeH_p1_1"+year,"p1_1"+year,165.0, 145.0, 185.0)    
        p3_1 = RooRealVar("CMS_fakeH_p3_1"+year,"p3_1"+year,89.0, 84.0,94.0) 
        p2_1 = RooFormulaVar("CMS_fakeH_p2_1"+year,"p2_1"+year,"0.72*@0-@1",RooArgList(p1_1,p3_1))    
        fakeH = RooLandau("fakeH", "landau", m, p1_1, p2_1)
    if (channel=='4e'):
        p1_2 = RooRealVar("CMS_fakeH_p1_2"+year,"p1_2"+year,165.0, 145.0, 185.0)    
        p3_2 = RooRealVar("CMS_fakeH_p3_2"+year,"p3_2"+year,89.0, 84.0,94.0) 
        p2_2 = RooFormulaVar("CMS_fakeH_p2_2"+year,"p2_2"+year,"0.72*@0-@1",RooArgList(p1_2,p3_2))    
        fakeH = RooLandau("fakeH", "landau", m, p1_2, p2_2) 
    if (channel=='2e2mu'):
        p1_3 = RooRealVar("CMS_fakeH_p1_3"+year,"p1_3"+year,165.0, 145.0, 185.0)    
        p3_3 = RooRealVar("CMS_fakeH_p3_3"+year,"p3_3"+year,89.0, 84.0,94.0) 
        p2_3 = RooFormulaVar("CMS_fakeH_p2_3"+year,"p2_3"+year,"0.72*@0-@1",RooArgList(p1_3,p3_3))    
        fakeH = RooLandau("fakeH", "landau", m, p1_3, p2_3)
    '''
...
#                                  `SM_125_2e2mu_pT4l_genbin8_recobin0
    ...
    frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_'+obsName+'_'+recobin+bkg]
    ...
    frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_'+obsName+'_'+recobin+bkg]
    ...
    #  FIXME there should be yearly "templates" in respective directories e.g. templates_2018
...
#    data_obs_file = TFile('data_13TeV_2018.root')  # David PAS
    '''
    if (year=="2018"): data_obs_file = TFile('data_13TeV_2018_Jun05.root')   # createdata.C output with Qianying sample 
    elif (year=="2017"): data_obs_file = TFile('data_13TeV_2017_newJun05.root')   # createdata.C output with Qianying sample 
    else: data_obs_file = TFile('data_13TeV_newJul29.root')   # createdata.C output with sample 
    '''
    if (year=="2018"): data_obs_file = TFile('/afs/cern.ch/user/t/tjavaid/workspace/UL_Ana/CMSSW_10_2_13/src/FidXS_13TeV_combination_ZZnorm/data_13TeV_2018_Jun05.root')   #   
    elif (year=="2017"): data_obs_file = TFile('/afs/cern.ch/user/t/tjavaid/workspace/UL_Ana/CMSSW_10_2_13/src/FidXS_13TeV_combination_ZZnorm/data_13TeV_2017_newJun05.root')   #   
    else : data_obs_file = TFile('/afs/cern.ch/user/t/tjavaid/workspace/UL_Ana/CMSSW_10_2_13/src/FidXS_13TeV_combination_ZZnorm/data_13TeV_newJul29.root')   #   
    ...
    if (channel=='4mu'):
        if (obsName.startswith("mass4l")): data_obs = RooDataSet("data_obs","data_obs",data_obs_tree,RooArgSet(m,mass4mu),"(mass4mu>105.0 && mass4mu<160.0)")
        elif (obsName.startswith("abs")):  data_obs = RooDataSet("data_obs","data_obs",data_obs_tree,RooArgSet(m,mass4mu,observable),"(mass4mu>105.0 && mass4mu<160.0 && "+obsName+">="+obsBin_low+" && "+obsName+"<"+obsBin_high+")")
        else: data_obs = RooDataSet("data_obs","data_obs",data_obs_tree,RooArgSet(m,mass4mu,observable),"(mass4mu>105.0 && mass4mu<160.0 && abs("+obsName+")>="+obsBin_low+" && abs("+obsName+")<"+obsBin_high+")")
    if (channel=='4e'):
        if (obsName.startswith("mass4l")): data_obs = RooDataSet("data_obs","data_obs",data_obs_tree,RooArgSet(m,mass4e),"(mass4e>105.0 && mass4e<"+str(mass4l_high)+")")
        elif (obsName.startswith("abs")):  data_obs = RooDataSet("data_obs","data_obs",data_obs_tree,RooArgSet(m,mass4e,observable),"(mass4e>105.0 && mass4e<"+str(mass4l_high)+" && "+obsName+">="+obsBin_low+" && "+obsName+"<"+obsBin_high+")")
        else: data_obs = RooDataSet("data_obs","data_obs",data_obs_tree,RooArgSet(m,mass4e,observable),"(mass4e>105.0 && mass4e<"+str(mass4l_high)+" && abs("+obsName+")>="+obsBin_low+" && abs("+obsName+")<"+obsBin_high+")")
    if (channel=='2e2mu'):
        if (obsName.startswith("mass4l")): data_obs = RooDataSet("data_obs","data_obs",data_obs_tree,RooArgSet(m,mass2e2mu),"(mass2e2mu>105.0 && mass2e2mu<"+str(mass4l_high)+")")
        elif (obsName.startswith("abs")):  data_obs = RooDataSet("data_obs","data_obs",data_obs_tree,RooArgSet(m,mass2e2mu,observable),"(mass2e2mu>105.0 && mass2e2mu<"+str(mass4l_high)+" && "+obsName+">="+obsBin_low+" && "+obsName+"<"+obsBin_high+")")
        else: data_obs = RooDataSet("data_obs","data_obs",data_obs_tree,RooArgSet(m,mass2e2mu,observable),"(mass2e2mu>105.0 && mass2e2mu<"+str(mass4l_high)+" && abs("+obsName+")>="+obsBin_low+" && abs("+obsName+")<"+obsBin_high+")") 

    print "data_obs: ",data_obs.numEntries(), "mass4l_low:  ", mass4l_low, "mass4l_high: ",mass4l_high  
    ...
    if (addfakeH):
        if (usecfactor):
            ...
            fout = TFile("xs_125.0_"+year+bkg+"/hzz4l_"+channel+"S_13TeV_xs_"+modelName+"_"+obsName+"_"+physicalModel+".Databin"+str(obsBin)+".Cfactor.root","RECREATE")
        else:
            fout = TFile("xs_125.0_"+year+bkg+"/hzz4l_"+channel+"S_13TeV_xs_"+modelName+"_"+obsName+"_"+physicalModel+".Databin"+str(obsBin)+".root","RECREATE")
    else:
        if (usecfactor):
            fout = TFile("xs_125.0_"+year+bkg+"/hzz4l_"+channel+"S_13TeV_xs_"+modelName+"_"+obsName+"_"+physicalModel+".Databin"+str(obsBin)+".Cfactor.NoFakeH.root","RECREATE")
        else:
            fout = TFile("xs_125.0_"+year+bkg+"/hzz4l_"+channel+"S_13TeV_xs_"+modelName+"_"+obsName+"_"+physicalModel+".Databin"+str(obsBin)+".NoFakeH.root","RECREATE")
...
#createXSworkspace("mass4l", "2e2mu", 2, 0, ["105.0","140.0"], False, True, "ggH_powheg_JHUgen_125", "v2","2017")
#createXSworkspace("mass4l", "2e2mu", 2, 0, ["105.0","160.0"], False, True, "ggH_powheg_JHUgen_125", "v2","2017","zz")
#createXSworkspace("mass4l", "2e2mu", 2, 0, ["105.0","160.0"], False, True, "ggH_powheg_JHUgen_125", "v2","2018","zz")
